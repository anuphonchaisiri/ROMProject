...
$.fn.smartSearch = function (datas) {
    ...
    formPopUp.find(".smart-search-popup-result-row").unbind("click").bind("click", function (e) {
        ...
        if ($(this).attr("data-allowevent") == "true") {
            if ($(this).attr("data-event") == "main") {
                $("div.smart-other-delegate-temp[data-code=" + code + "] span.input-group-addon").click();
            } else if ($(this).attr("data-event") == "other") {
                $("div.smart-main-delegate-temp[data-code=" + code + "] span.input-group-addon").click();
            }
        }
...
//custome
function SelectedSearchBoxOtherDelegate(event) {
    $('#txtSmartSearchOtherDelegate').click();
    event.stopPropagation();
}


function addSelectedSmartOtherDelegate(code, value, isContinue) {
    if ($(".smart-other-delegate-result [data-code='" + code + "']").length == 0) {
        var temp = $(".smart-other-delegate-template .smart-other-delegate-temp").clone();
        temp.find(".form-control").html(value);
        temp.attr("data-code", code);
        $(".smart-other-delegate-result").append(temp, $(".input-group-other-delegate-container"));

        resultSelectedSmartOtherDelegate();
        if (isContinue) {
            $('#txtSmartSearchOtherDelegate').focus();
        }
    }
}

function RemoveAllSelectOtherDelegate(obj) {
    var target = $(obj).closest(".panel-smart-other-delegate-template");
    $(target).find('.smart-other-delegate-result .smart-other-delegate-temp').remove();
    $("#txtCreateActivityOtherID").val("");
    $(target).find("#txtCreateActivityOtherID").val("");
}

function resultSelectedSmartOtherDelegate() {
    $("#txtCreateActivityOtherID").val("");
    var arr = [];
    $(".smart-other-delegate-result .smart-other-delegate-temp").each(function () {
        arr.push($(this).attr("data-code"));
    });
    $("#txtCreateActivityOtherID").val(arr.join(','));
}

function clearSelectedDelegateSmartSearch() {
    $(".smart-main-delegate-result .smart-main-delegate-temp").remove();
    $(".smart-other-delegate-result .smart-other-delegate-temp").remove();
    $("#txtCreateActivityMainID").val("");
    resultSelectedSmartOtherDelegate();
}
